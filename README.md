# Introduction To Embedded Systems

This Github repository is used to store the exercises completed in the course Introduction To Embedded Systems / Johdatus sulautettuihin järjestelmiin.

The course will be done so that the master branch will initially only contain this README.md and each exercise will be stored in a separate Git branch.

Branch naming and commit messaging will be standardized. Master commits will begin with **embedded-systems: \<commit-mgs>**. The exercise branches are named according to the exercise number, e.g. **f/EXERCISE-1** and exercise commit messages will be in the following format: **EXERCISE-1: \<commit-mgs>**

Once all of the exercises are done and the course is complete, all exercise branches will be in one branch, **f/EXERCISES**. The separate exercise branches will be kept in any case.

### Folders / branches

```
├── README.md
├── exercise_2 | (f/EXERCISE-2)/
|   ├── exercise_2_circuit_diagram.png
|   ├── exercise_2_part_1.ino
|   ├── exercise_2_part_2.ino
|   ├── exercise_2_part_3.ino
|   ├── harjoite_2_raportti.docx
|   └── harjoite_2_raportti.pdf
├── exercise_3 | (f/EXERCISE-3)/
|   ├── exercise_3_circuit_diagram.png
|   ├── exercise_3_part_1.ino
|   ├── exercise_3_part_2.ino
|   ├── exercise_3_part_3.ino
|   ├── harjoite_3_raportti.docx
|   └── harjoite_3_raportti.pdf
├── exercise_4 | (f/EXERCISE-4)/
|   ├── exercise_4_circuit_diagram.png
|   ├── exercise_4_part_1.ino
|   ├── exercise_4_part_2.ino
|   ├── exercise_4_part_3.ino
|   ├── harjoite_4_raportti.docx
|   └── harjoite_4_raportti.pdf
└── exercise_5 | (f/EXERCISE-5)/
    ├── exercise_5_part_1_circuit_diagram.png
    ├── exercise_5_part_1.ino
    ├── exercise_5_part_2_circuit_diagram.png
    ├── exercise_5_part_2.ino
    ├── exercise_5_part_3_circuit_diagram.png
    ├── exercise_5_part_3.ino
    ├── harjoite_5_raportti.docx
    └── harjoite_5_raportti.pdf
```